...
import type { DayType, TransitRoute, TransitStop } from "@/lib/transitData";
...
export default function TripPlanner() {
  ...
  const reverseGeocode = useCallback(
    async (coords: { lat: number; lng: number }): Promise<string> => {
  ...
  const getWalkingPath = useCallback(
    async (
      ...
      err => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            msg =
              "Location permission denied. Please enable location services in your browser/device settings and try again.";
            break;
  ...
  const findRoutes = useCallback(async () => {
    ...
    try {
      ...
      const patternsForStop = (stopId: string) =>
        patternCandidates.filter(pattern =>
          pattern.stops.some(stop => stop.id === stopId)
        );
      ...
      for (const pattern of patternCandidates) {
        const originMatch = findClosestStopInSequence(
          origin.lat,
          origin.lng,
          pattern.stops
        );
        const destMatch = findClosestStopInSequence(
          dest.lat,
          dest.lng,
          pattern.stops
        );
        ...
        const stopsBetween = pattern.stops.slice(
          originMatch.index,
          destMatch.index + 1
        );
        ...
        if (routeSched) {
          ...
          for (const trip of trips) {
            if (pattern.tripIds.size > 0 && !pattern.tripIds.has(trip.trip_id))
              continue;

            const originStopTime = trip.stops.find(
              s => s.stop_id === originMatch.stop.id
            );
            const destStopTime = trip.stops.find(
              s => s.stop_id === destMatch.stop.id
            );

            if (originStopTime && destStopTime) {
              const depMin = parseGtfsTime(
                originStopTime.departure
              ).totalMinutes;
        ...
        segments.push({
          ...
          distance: haversine(
            originMatch.stop.lat,
            originMatch.stop.lon,
            destMatch.stop.lat,
            destMatch.stop.lon
          ),
      ...
      if (tripResults.length < 3) {
        for (const nearOrig of nearOriginStops.slice(0, 8)) {
          ...
          for (const origPattern of origPatterns) {
            const origStopIdx = origPattern.stops.findIndex(
              s => s.id === nearOrig.stop.id
            );
            if (origStopIdx < 0) continue;

            for (
              let ti = origStopIdx + 1;
              ti < origPattern.stops.length;
              ti += 1
            ) {
              ...
              for (const destPattern of transferPatterns) {
                ...
                const transferStopIdx = destPattern.stops.findIndex(
                  s => s.id === transferStop.id
                );
                if (transferStopIdx < 0 || transferStopIdx >= destMatch.index)
                  continue;
                ...
                const color1 =
                  routeColors.get(origPattern.route.id) || "#00D4FF";
                const color2 =
                  routeColors.get(destPattern.route.id) || "#FFB020";
                const stopsCount1 = ti - origStopIdx;
                const stopsCount2 = destMatch.index - transferStopIdx;

                // Get stops between for each leg
                const stopsBetween1 = origPattern.stops.slice(
                  origStopIdx,
                  ti + 1
                );
                const stopsBetween2 = destPattern.stops.slice(
                  transferStopIdx,
                  destMatch.index + 1
                );
                ...
                const totalDist = segments.reduce(
      ...
      for (const r of tripResults) {
        const isDupe = filtered.some(
          f =>
            f.label === r.label &&
            Math.abs(f.totalDuration - r.totalDuration) < 5
        );
        if (!isDupe) {
          filtered.push(r);
        }
      ...
      if (filtered.length === 0) {
        if (directDist > 5) {
          toast.info(
            `No transit routes found for this ${formatDistance(directDist)} trip. The distance may be too far for current bus coverage.`
          );
        } else {
          toast.info(
            "No transit routes found. Try adjusting your origin or destination."
          );
        }
      } else {
        toast.success(
          `Found ${filtered.length} route option${filtered.length > 1 ? "s" : ""}!`
        );
      ...
      const drawSegments = async () => {
        for (let segIdx = 0; segIdx < result.segments.length; segIdx++) {
          ...
          if (seg.type === "bus" && seg.route && seg.fromStop && seg.toStop) {
            ...
            const slicedShape = sliceRouteShape(
              seg.route,
              seg.fromStop,
              seg.toStop
            );
              ...
              seg.stopsBetween.forEach((stop, i) => {
                ...
                marker.addListener("click", () => {
                  if (infoWindowRef.current) {
                    ...
                    if (isBoarding)
                      statusLabel =
                        '<span style="color:#00FF88;font-weight:bold;">BOARDING STOP</span>';
                    else if (isAlighting)
                      statusLabel =
                        '<span style="color:#FF4444;font-weight:bold;">EXIT STOP</span>';
                    else
                      statusLabel = `<span style="color:${color};">Stop ${stopNum} of ${totalStops}</span>`;
            ...
            if (seg.fromName === "Your Location" && originCoords) {
              ...
              if (nextSeg?.fromStop) {
                walkTo = {
                  lat: nextSeg.fromStop.lat,
                  lng: nextSeg.fromStop.lon,
                };
              }
  ...
  return (
    <div className="h-[100dvh] w-screen overflow-hidden bg-background relative">
      ...
      <div
        className="absolute inset-0 top-14 map-container"
        style={{ zIndex: 1 }}
      >
        <MapView
      ...
      >
        <Crosshair
          className="w-4 h-4"
          style={{ color: dropMode === "origin" ? "#788c5d" : "#d97757" }}
        />
        <span
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          {selectedResult !== null &&
            results[selectedResult] &&
            results[selectedResult].segments.some(
              s => s.type === "walk" && s.distance > 0.03
            ) && (
        ...
        <div className="p-3 md:p-4 border-b border-border/50 shrink-0">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-1.5 ml-5">
              ...
              <button
                onClick={() => {
                  setDropMode(dropMode === "origin" ? null : "origin");
                  if (dropMode !== "origin")
                    toast.info("Tap the map to set your starting point");
                }}
            ...
            <div className="flex gap-1.5 ml-5">
              <button
                onClick={() => {
                  setDropMode(dropMode === "dest" ? null : "dest");
                  if (dropMode !== "dest")
                    toast.info("Tap the map to set your destination");
                }}
                className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] transition-colors"
                style={{
                  fontFamily: "'Space Grotesk', system-ui, sans-serif",
                      ...
                      .filter(
                        s =>
                          !(
                            ...
                            s.distance === 0 &&
                            s.duration <= 5 &&
                            s.fromName.startsWith("Transfer")
                          )
                      )
                      .map((seg, j) => (
                        <div key={j} className="flex items-center gap-1">
                          ...
                          {seg.type === "walk" ? (
                            ...
                            >
                              ...
                              {seg.stopsCount !== undefined && (
                                <span className="opacity-70">
                                  ({seg.stopsCount} stops)
                                </span>
                              )}
                            </div>
                          )}
                      ...
                      >
                            ...
                            <div className="flex-1">
                              ...
                              {seg.departureTime && (
                                <div
                                  ...
                                  style={{ color: seg.color }}
                                >
                                  Departs {formatTime(seg.departureTime)} →
                                  Arrives {formatTime(seg.arrivalTime!)}
                                </div>
                              )}
                        ...
                        {result.walkOnly && (
                          <div className="mt-2 p-2 rounded bg-[#00FF88]/5 border border-[#00FF88]/20 text-[10px] font-mono text-[#00FF88]">
                            ...
                            {formatDistance(result.totalDistance)} in about{" "}
                            {formatDuration(result.totalDuration)}.
                          </div>
                        )}
                        {result.bikeOnly && (
                          <div className="mt-2 p-2 rounded bg-[#FFD700]/5 border border-[#FFD700]/20 text-[10px] font-mono text-[#FFD700]">
                            Bikeable — {formatDistance(result.totalDistance)} in
                            about {formatDuration(result.totalDuration)}.
                          </div>
                        )}
                      </motion.div>
                    )}
